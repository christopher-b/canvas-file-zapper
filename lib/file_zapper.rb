...
class FileZapper
  ...
  def wip_delete_errors
    # Clear out failed or half-completed uploads?
  ...
  def replace_submissions(term, also = [:comments, :quizzes])
    ...
    att_ids = Version.where(
      ...
      ids = YAML.safe_load(version.yaml)['attachment_ids'] || ''
      ids.split(',')
    end.flatten.compact
  ...
  def delete_submission_exports
    Attachment
      .where(
        ...
        display_name:   'submissions.zip' # Not strictly necessary, but possibly helpful later
      )
  ...
  def delete_epub_exports
    ...
    epubs = EpubExport.where('created_at < ?', @options[:cutoff_epubs])
                      .preload(:attachments, :content_export)
  ...
  def replace_files(att_ids)
    ...
    att_ids.each_slice(500) do |ids_batch|
      Attachment.where(id: ids_batch).each do |att|
        ...
        new_root = image?(att) ? root_image : root_pdf
  ...
  def delete_content_export_and_attachment(content_export)
    # ContentExport#destroy is broken: PG throws a FK violation when deleting the attachment row
